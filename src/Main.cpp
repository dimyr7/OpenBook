...
int main(int argc, char** argv){
	...
	while(true){
		std::cout << "openbook> ";
		std::string command;
		// TODO gets suck on this getline call
		...
		if(inputCommands[0] == "n"){
			...
			if(inputCommands.size() != 4){
				std::cout << "form: <n> <month> <year>" << std::endl;
			}
			...
			// <c> : close the most reason jounral

			if(journal == nullptr){
				std::cout << RED "A journal must be open before it can be close" RESET << std::endl;
			}
			...
			std::cerr << RED "Not recognized command" << std::endl;
		}
	}
